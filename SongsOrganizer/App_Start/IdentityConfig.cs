...
namespace SongsOrganizer
{
    ...
    public class UzytkownikManager : UserManager<User>
    {
        public UzytkownikManager(IUserStore<User> store)
            : base(store)
        {
        }
        ...
        {
            var manager = new UzytkownikManager(new UserStore<User>(context.Get<AppContext>()));
